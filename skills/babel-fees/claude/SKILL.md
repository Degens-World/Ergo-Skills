---
name: ergo-babel-fee-box-discovery
description: "A tool to discover and select Ergo Babel fee boxes (EIP-0031) for paying transaction fees in tokens instead of ERG. It finds on-chain liquidity boxes for a specific token and filters them by the required ERG amount."
---

# Ergo Babel Fee Box Discovery
...
## How to use Ergo Babel Fee Boxes # Babel Box Token Payments — Technical Reference
...
### Successfully finds Babel boxes for a known token (CYPX). This is the most common use case where a user wishes to pay a fee using a specific token and the tool looks for available liquidity boxes.
...
### Demonstrates the scenario where no Babel boxes are found for a given token ID. This can happen if the token is not supported or if all liquidity has been consumed. The tool returns an empty list.
...
### Shows how to use the 'required_nano_erg' parameter to find a Babel box with a specific minimum amount of ERG liquidity, useful for transactions that require more ERG than the standard fee.
...
- `ergo:public-api:boxes:read`
- `ergo:public-api:network-state:read`

## Constraints

- API Rate Limit: 10 requests per second to the public Ergo Explorer API.
- Timeout: 15 seconds per API request.
- The tool performs two API calls for each request to search for both ErgoTree header formats (`10...` and `18c101...`).
